  ...
  init: function () {
    this.handler = (e) => {
      ...
      if (h !== undefined && !this.aligned) {
        ...
        this.aligned = true;
        console.log("APP: Aligned to North: " + h);
      }
    };
...
AFRAME.registerComponent('gps-new-place', {
  ...
  begin: function() {
    if (!AFRAME.utils.device.isMobile()) {
      // Default central position for testing on desktop
...
window.teleportToExhibition = () => {
  ...
  window.xrState = "Teleported to Commotion Group";
};
...
window.updateDebugUI = () => {
  ...
  html += `<button onclick="location.reload()" style="width:100%; background:#28a745; color:white; border:none; margin:2px 0; padding:5px; font-weight:bold;">1. REFRESH PAGE</button>`;
  ...
  Object.keys(window.debugData).sort().forEach(k => {
    const e = window.debugData[k];
